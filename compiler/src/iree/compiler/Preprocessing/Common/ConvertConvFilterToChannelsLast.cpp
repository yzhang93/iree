...
struct ConvertGenericChwfToFhwc : public OpRewritePattern<linalg::GenericOp> {
  ...
  LogicalResult matchAndRewrite(linalg::GenericOp op,
    ...
    AffineMap transposedInputMap = applyPermutationToResults(inputMap, perm);
    Value transposedInput = createTransposeOp(rewriter, loc, inputVal, perm);
    ...
    auto genericOp = linalg::GenericOp::create(
        rewriter, loc, outputVal.getType(),
        ValueRange{transposedFilter, transposedInput}, outputVal,
        ArrayRef<AffineMap>{transposedFilterMap, transposedInputMap, outputMap},
        iterators);
