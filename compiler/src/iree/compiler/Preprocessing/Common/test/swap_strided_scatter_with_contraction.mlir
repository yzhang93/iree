// RUN: iree-opt --split-input-file --mlir-print-local-scope --iree-preprocessing-swap-strided-scatter-with-contraction %s | FileCheck %s

// Swapped: 1x1 backward conv with stride-2 scatter feeding a matmul-like
// contraction followed by truncf. The contraction moves before the scatter
// and operates on the small (un-scattered) source.
...
// CHECK-NOT:   tensor.insert_slice {{.*}} tensor<2x8x8x16xf16> into tensor<2x18x18x16xf16>
// Contraction operates on the small source (2x8x8, not 2x18x18).
...
// Scatter the small truncated result into zeros.
...
// Swapped: contraction without consumer chain (no truncf).
