...
void mlir::iree_compiler::gpu::ConvolutionImplicitGemmStrategy::configure(
  ...
  if (captures.convolutionDims.inputChannel.size() == 1 &&
    ...
    // int nSize =
    // captures.convolutionOpSizes[captured.convolutionDims.outputChannel.back()];
    ...
    reductionLoopTileSizes =
        SmallVector<int64_t>(captures.convolutionDims.batch.size() +
                                 captures.convolutionDims.outputChannel.size(),
                             0);
    // reductionLoopTileSizes.push_back(innerLoopTileSize);
    reductionLoopTileSizes.push_back(1);
...
void mlir::iree_compiler::gpu::buildConvolutionImplicitGemmStrategy(
  ...
  Value tiledImg2colH;
  if (strategy.doIm2Col)
    tiledImg2colH = b.create<FuseIntoContainingOp>(img2colH, tileResult.forallH)
                        .getResult();
  ...
  if (strategy.doIm2Col) {
    iree_compiler::buildTileFuseDistToForallWithNumThreads(
        ...
        /*numThreads=*/
        getAsOpFoldResult(
            b.getI64ArrayAttr(strategy.getIm2ColThreadTileSizes())),
