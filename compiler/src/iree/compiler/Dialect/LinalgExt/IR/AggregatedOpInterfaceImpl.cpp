...
FailureOr<SmallVector<Value>> Im2colOp::decomposeOperation(OpBuilder &b) {
  ...
  // Check if there is padding along the contigous dimension, which would be the
  // first dimension when `batch` dimension is contiguous.
  bool fusePad = false;
  ...
  bool contiguousAlongK = getKPos().back() == inputInnerDim;
  bool contiguousAlongB = singleBatchDimInnermost;
  if (contiguousAlongK) {
  ...
  // For padding case, set static output to use for scf.for loop nest.
  ...
  // For pad cases, calculate the dynamic slice size.
  if (fusePad) {
